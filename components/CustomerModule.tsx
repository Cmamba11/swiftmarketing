    ...
    <div className="space-y-6">
      <div className="bg-white p-5 rounded-3xl border border-slate-200 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <input type="text" placeholder="Search by name..." value={searchTerm} onChange={(e) => onSearchChange(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#E31E24] outline-none transition font-bold" />
        </div>
        {canCreate && (
          <button onClick={() => setShowAdd(!showAdd)} className="flex items-center justify-center gap-2 px-8 py-3 bg-[#1A2B6D] text-white rounded-xl font-black uppercase tracking-widest hover:bg-[#E31E24] transition shadow-lg active:scale-95">
      ...
      {showAdd && canCreate && (
        <div className="bg-white p-10 rounded-[3rem] border-2 border-[#1A2B6D] shadow-2xl animate-in slide-in-from-top-4">
          <h3 className="text-xl font-black italic uppercase tracking-tighter mb-8 flex items-center gap-3 text-[#1A2B6D] border-b border-slate-100 pb-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button type="submit" className="w-full py-4 bg-[#E31E24] text-white rounded-xl font-black uppercase tracking-widest hover:bg-[#1A2B6D] transition shadow-xl">
          ...
          <div key={customer.id} className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm hover:shadow-xl transition-all group flex flex-col">
            <div className="flex justify-between items-start mb-6">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 bg-slate-50 rounded-3xl flex items-center justify-center text-[#1A2B6D] group-hover:bg-[#E31E24] group-hover:text-white transition-colors">
                  <Building2 size={28} />
                </div>
                <div>
                  <h4 className="text-xl font-black text-slate-900 uppercase italic tracking-tighter leading-tight">{customer.name}</h4>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1"><MapPin size={12} className="text-[#E31E24]" /> {customer.location}</p>
                </div>
              </div>
            ...
            <div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-6">
              ...
              <div className="flex gap-2">
                {canDelete && (
                  <button onClick={() => onDelete(customer.id)} className="p-2.5 bg-slate-50 rounded-xl text-slate-400 hover:text-[#E31E24] transition border border-transparent hover:border-slate-200">
                    <Trash2 size={16} />
                  </button>
                )}
              </div>
