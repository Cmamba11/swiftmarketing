  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-5 rounded-3xl border border-slate-200 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
        ...
        {canCreate && (
          <button onClick={() => setShowAdd(!showAdd)} className="flex items-center justify-center gap-2 px-8 py-3 bg-swift-navy text-white rounded-xl font-black uppercase tracking-widest hover:bg-swift-red transition">
      ...
      {showAdd && (
        <form onSubmit={handleSubmit} className="bg-white p-10 rounded-[3rem] border-2 border-swift-navy shadow-2xl animate-in slide-in-from-top-4 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <input placeholder="Partner Enterprise Name" className="p-4 border rounded-xl" onChange={e => setFormData({...formData, name: e.target.value})} required />
            <input placeholder="Location (City)" className="p-4 border rounded-xl" onChange={e => setFormData({...formData, location: e.target.value})} required />
            <input placeholder="Tax ID" className="p-4 border rounded-xl" onChange={e => setFormData({...formData, taxId: e.target.value})} required />
            <input placeholder="Contact Head" className="p-4 border rounded-xl" onChange={e => setFormData({...formData, contactPerson: e.target.value})} required />
          </div>
            ...
            <div key={partner.id} className="bg-white rounded-[3rem] border border-slate-200 p-8 shadow-sm hover:shadow-xl transition-all">
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-slate-100 rounded-3xl flex items-center justify-center text-swift-navy font-black text-2xl uppercase">{partner.name.charAt(0)}</div>
                  <div>
                    <h4 className="text-xl font-black text-slate-900 uppercase italic tracking-tighter">{partner.name}</h4>
                ...
                <span className="bg-blue-50 text-blue-600 px-3 py-1.5 rounded-xl text-[8px] font-black uppercase">Active Partner</span>
              </div>

              <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 mb-6">
                <h5 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 flex items-center gap-2"><PackageOpen size={14} className="text-swift-red" /> Partner Specific Assets (Rollers)</h5>
                {partnerAssets.length > 0 ? (
                  <div className="space-y-3">
                    {partnerAssets.map(asset => (
                      <div key={asset.id} className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm">
                        <span className="text-xs font-black uppercase italic text-slate-700">{asset.productName}</span>
                        <div className="flex items-center gap-4">
                          {asset.totalKg && (
                          ...
                          <span className="text-lg font-black text-swift-navy">{asset.quantity} <span className="text-[8px] text-slate-400">ROLLS</span></span>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-[10px] italic text-slate-400 font-bold uppercase tracking-widest">No custom assets deployed.</p>
                )}
              </div>

              <div className="flex justify-end gap-2 border-t border-slate-100 pt-6">
                {canDelete && (
                  <button onClick={() => onDelete(partner.id)} className="p-3 text-slate-300 hover:text-swift-red transition"><Trash2 size={18} /></button>
                )}
              </div>
            </div>
          );
